...
if __name__ == '__main__':
    ...
    bim = Attack_BasicIterativeMethod(eps=0.3, eps_iter=0.06, nb_iter=2)
    X_test_adv = bim.attack(model, X_test, Y_test)
    bim2 = Attack_BasicIterativeMethod(eps=0.3, eps_iter=0.06, nb_iter=5)
    X_test_adv2 = bim2.attack(model, X_test, Y_test)
    bim3 = Attack_BasicIterativeMethod(eps=0.3, eps_iter=0.06, nb_iter=10)
    time_start = time.time()
    X_test_adv3 = bim3.attack(model, X_test, Y_test)
    dur_per_sample = (time.time() - time_start) / len(X_test_adv)
    for i in range(10):
        save_img(os.path.join("./pic2/", str(i)+"1.jpeg"), X_test[i])
        save_img(os.path.join("./pic2/",str(i)+"2.jpeg"), X_test_adv[i])
        save_img(os.path.join("./pic2/",str(i)+"3.jpeg"), X_test_adv2[i])
        save_img(os.path.join("./pic2/",str(i)+"4.jpeg"), X_test_adv3[i])
        print(Y_test[i])
        print("----")
    dur_per_sample = (time.time() - time_start) / len(X_test_adv)
